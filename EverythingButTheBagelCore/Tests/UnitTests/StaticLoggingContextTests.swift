...
class StaticLoggingContextTests: XCTestCase {
  ...
  func testLog() throws {
    ...
    let logCategory = TestLoggingContext.loggingCategory
    ...
    withTestClient { context, setupSpy, logSpy in
      ...
      XCTAssertEqual(logSpy.callParams[0].1, logCategory)
    }
  }
  ...
  private func withTestClient(closure: (TestLoggingContext.Type, Spy, Spy2<LogLevel, String>) throws -> Void) rethrows {
    ...
    try withDependencies { dependencies in
      ...
      try closure(TestLoggingContext.self, setupSpy, logSpy)
    }
  }

  private func withTestClient(closure: (TestLoggingContext.Type, Spy, Spy2<LogLevel, String>) async throws -> Void) async rethrows {
    let (setupSpy, setupFn) = spy({})
    let (logSpy, logFn) = spy({ (_: LogLevel, _: String) in })
    ...
    try await withDependencies { dependencies in
      ...
      try await closure(TestLoggingContext.self, setupSpy, logSpy)
    }
  }
}

private struct TestLoggingContext: StaticLoggingContext {
  static let loggingCategory = "Test"
}
